...
router.put('/masters/:id/subscription', async (req, res) => {
    try {
        ...
        let sub = await prismaClient_1.default.subscription.findUnique({ where: { masterId } });
...
router.get('/statistics', async (req, res) => {
    try {
        const totalMasters = await prismaClient_1.default.user.count({ where: { role: 'MASTER' } });
        const totalClients = await prismaClient_1.default.user.count({ where: { role: 'CLIENT' } });
        const activePro = await prismaClient_1.default.subscription.count({
            where: { plan: 'PRO', status: 'ACTIVE' }
        });
        const activeTrials = await prismaClient_1.default.subscription.count({
            where: { plan: 'PRO', status: 'TRIAL' }
        });
        const canceledOrExpired = await prismaClient_1.default.subscription.count({
            where: { status: { in: ['EXPIRED', 'CANCELLED'] } }
        });
        const successfulPayments = await prismaClient_1.default.payment.aggregate({
...
router.get('/config', async (req, res) => {
    try {
        const configs = await prismaClient_1.default.systemConfig.findMany();
        const configMap = {};
        configs.forEach(c => configMap[c.key] = c.value);
        // Return default PRO_PRICE if not explicitly saved yet
        if (!configMap['PRO_PRICE'])
            configMap['PRO_PRICE'] = '299';
        res.json(configMap);
    }
    catch (e) {
        res.status(500).json({ error: 'Server error' });
...
router.post('/config', async (req, res) => {
    try {
        ...
        const config = await prismaClient_1.default.systemConfig.upsert({
            where: { key },
...
router.put('/users/:id/ban', async (req, res) => {
    try {
        ...
        const u = await prismaClient_1.default.user.update({
