...
const requireMaster = (req, res, next) => {
    if (req.user?.role !== 'MASTER')
        return res.status(403).json({ error: 'Not a master' });
    next();
};
router.use(authAny);
...
router.post('/connection-code', async (req, res) => {
    ...
    try {
        ...
        await prismaClient_1.default.connectionCode.upsert({
...
router.get('/settings', async (req, res) => {
    ...
    const settings = await prismaClient_1.default.masterWeeklySettings.findMany({
        where: { masterId },
...
router.put('/settings', async (req, res) => {
    ...
    try {
        ...
        await prismaClient_1.default.masterWeeklySettings.deleteMany({ where: { masterId } });
        const formattedData = weeklySettings.map((s) => ({
            masterId,
        ...
        await prismaClient_1.default.masterWeeklySettings.createMany({
            data: formattedData
        });
        const newSettings = await prismaClient_1.default.masterWeeklySettings.findMany({ where: { masterId } });
...
router.get('/slots', async (req, res) => {
    ...
    if (dayOfWeek === 0)
        dayOfWeek = 7; // Convert Sun(0) to 7
...
router.post('/slots/block', async (req, res) => {
    ...
    const blocked = await prismaClient_1.default.blockedSlot.create({
        data: { masterId: req.user.id, date: new Date(date), time, reason }
    });
    res.json(blocked);
});
// DELETE /slots/unblock
router.delete('/slots/unblock', async (req, res) => {
    const { date, time } = req.body; // Can send in body or query
    await prismaClient_1.default.blockedSlot.deleteMany({
...
router.put('/appointments/archive-old', async (req, res) => {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const updated = await prismaClient_1.default.appointment.updateMany({
        where: {
            masterId: req.user.id,
            date: { lt: today },
            isArchived: false
...
router.put('/appointments/:id/confirm', async (req, res) => {
    ...
    const paymentInfo = await prismaClient_1.default.paymentInfo.findUnique({ where: { masterId } });
    ...
    if (note && !isPrepayment)
        msg += ` Примітка майстра: ${note}`;
    (0, telegram_1.sendTelegramMessage)(app.clientId, msg);
    if (app.client?.pushToken?.token) {
        await (0, firebase_1.sendPushNotification)(app.client.pushToken.token, isPrepayment ? 'Очікується передоплата' : 'Запис підтверджено', msg);
    }
    // Socket emit
    const socketId = (0, index_1.getSocketId)(app.clientId);
    if (socketId) {
        (0, index_1.getIo)().to(socketId).emit('appointment_updated', app);
    }
    res.json(app);
});
...
router.put('/appointments/:id/cancel', async (req, res) => {
    ...
    (0, telegram_1.sendTelegramMessage)(app.clientId, `Ваш запис на ${app.date.toISOString().split('T')[0]} о ${app.time} СКАСОВАНО.`);
    if (app.client?.pushToken?.token) {
        await (0, firebase_1.sendPushNotification)(app.client.pushToken.token, 'Запис скасовано', `Майстер ${app.master?.name || ''} відхилив Ваш запис на ${app.date.toISOString().split('T')[0]}.`);
    }
    res.json(app);
});
...
router.get('/statistics', async (req, res) => {
    ...
    if (period && date) {
        ...
        if (period === 'week') {
            gte = new Date(d);
            gte.setDate(d.getDate() - d.getDay() + 1);
            lte = new Date(gte);
    ...
    apps.forEach((a) => {
        ...
        if (a.service) {
            if (!serviceCount[a.service])
                serviceCount[a.service] = { count: 0, income: 0 };
...
router.put('/payment-details', async (req, res) => {
    const masterId = req.user.id;
    const { cardNumber, bankName, paymentLink, requirePrepaymentGlobal, globalPrepaymentAmount, liqpayPublicKey, liqpayPrivateKey } = req.body;
    const updateObj = {
        cardNumber,
        bankName,
        fullName: '',
    ...
    const p = await prismaClient_1.default.paymentInfo.upsert({
        where: { masterId },
        update: updateObj,
...
router.post('/blocked-phones', authAny, requireMaster, async (req, res) => {
    try {
        const { phone, reason } = req.body;
        if (!phone)
            return res.status(400).json({ error: 'phone is required' });
        const blocked = await prismaClient_1.default.blockedPhone.create({
...
router.get('/blocked-phones', authAny, requireMaster, async (req, res) => {
    try {
        const list = await prismaClient_1.default.blockedPhone.findMany({ where: { masterId: req.user.id } });
        res.json(list);
    }
    catch (e) {
...
router.post('/prices', async (req, res) => {
    ...
    if (isFree) {
        const pricesCount = await prismaClient_1.default.priceList.count({ where: { masterId: req.user.id } });
        if (pricesCount >= 1) {
            return res.status(403).json({ error: 'Ліміт тарифу FREE: лише 1 послуга. Оновіть до PRO.' });
        }
    }
    const p = await prismaClient_1.default.priceList.create({
...
router.put('/prices', async (req, res) => {
    ...
    const p = await prismaClient_1.default.priceList.update({
        where: { id },
...
router.delete('/prices/:id', async (req, res) => {
    ...
    try {
        await prismaClient_1.default.priceList.delete({ where: { id: req.params.id } });
        res.json({ success: true });
    }
    catch (e) {
...
router.get('/clients', async (req, res) => {
    try {
        const clients = await prismaClient_1.default.user.findMany({
...
router.get('/clients/:id', async (req, res) => {
    try {
        const client = await prismaClient_1.default.user.findFirst({
...
router.put('/clients/:id', async (req, res) => {
    try {
        ...
        const client = await prismaClient_1.default.user.findFirst({ where: { id: req.params.id, masterId: req.user.id } });
        if (!client)
            return res.status(404).json({ error: 'Client not found' });
...
// ==================== SALON BRANDING ====================
// PUT /salon-info
router.put('/salon-info', async (req, res) => {
    try {
        const { salonName, salonLogo, lat, lng, instagram, tiktok, facebook } = req.body;
        const updateData = {};
        if (salonName !== undefined)
            updateData.salonName = salonName;
        if (salonLogo !== undefined)
            updateData.salonLogo = salonLogo;
        if (lat !== undefined)
