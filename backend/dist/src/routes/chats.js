...
router.get('/', async (req, res) => {
    try {
        ...
        const userAppointments = await prismaClient_1.default.appointment.findMany({
            where: { OR: [{ masterId: req.user.id }, { clientId: req.user.id }] },
            select: { id: true }
        });
        const appointmentIds = userAppointments.map(a => a.id);
        const chats = await prismaClient_1.default.chat.findMany({
            where: {
                OR: [
        ...
        const enrichedChats = await Promise.all(chats.map(async (chat) => {
            ...
            if (chat.roomId && chat.roomId.includes('_')) {
                ...
                if (otherId) {
                    otherUser = await prismaClient_1.default.user.findUnique({
                        where: { id: otherId },
                ...
                if (appointment) {
                    if (req.user.role === 'MASTER')
                        otherUser = appointment.client;
                    else
                        otherUser = appointment.master;
                }
            }
...
router.get('/:roomId/messages', async (req, res) => {
    try {
        const chat = await prismaClient_1.default.chat.findUnique({
...
router.post('/:roomId/messages', async (req, res) => {
    try {
        ...
        let chat = await prismaClient_1.default.chat.findUnique({
            where: { roomId: req.params.roomId }
        });
        if (!chat) {
            chat = await prismaClient_1.default.chat.create({
