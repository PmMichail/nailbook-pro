...
router.post('/register', async (req, res) => {
    try {
        ...
        const existingUser = await prismaClient_1.default.user.findUnique({ where: { phone } });
        if (existingUser) {
        ...
        if (role !== 'MASTER' && effectiveReferralCode) {
            validReferralCodeObj = await prismaClient_1.default.referralCode.findUnique({
        ...
        if (userRole === 'MASTER') {
            ...
            if (/^\d{6}$/.test(inviteCode)) {
                const connection = await prismaClient_1.default.connectionCode.findUnique({ where: { code: inviteCode } });
                if (connection && new Date() <= connection.expiresAt) {
                    master = await prismaClient_1.default.user.findUnique({ where: { id: connection.userId } });
                }
            }
            // Fallback to permanent linkSlug if no master found yet
            if (!master) {
                master = await prismaClient_1.default.user.findUnique({ where: { linkSlug: inviteCode } });
            }
            if (master) {
                resolvedMasterId = master.id;
                // Enforce Subscription Limits
                const sub = await prismaClient_1.default.subscription.findUnique({ where: { masterId: master.id } });
        ...
        if (validReferralCodeObj && !resolvedMasterId) {
            const owner = await prismaClient_1.default.user.findUnique({ where: { id: validReferralCodeObj.clientId } });
        ...
        if (validReferralCodeObj) {
            await prismaClient_1.default.referralUse.create({
                data: {
            ...
            await prismaClient_1.default.referralCode.update({
        ...
        const token = jsonwebtoken_1.default.sign({ id: user.id, role: user.role, masterId: user.masterId }, JWT_SECRET, { expiresIn: '7d' });
...
router.post('/login', async (req, res) => {
    try {
        ...
        if (phone.includes('@')) {
            // Log in by email
            user = await prismaClient_1.default.user.findFirst({ where: { email: phone } });
        }
        else {
            // Log in by phone
            const fPhone = formatPhoneNumber(phone);
            user = await prismaClient_1.default.user.findFirst({ where: { phone: fPhone } });
        }
        if (!user) {
